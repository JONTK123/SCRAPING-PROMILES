...
class AzulScraper(BaseScraper):
    """Scraper for Azul Airlines"""
    
    def __init__(self):
        super().__init__(
            airline_name="AZUL",
            base_url="https://www.voeazul.com.br/home/br/pt/home"
        )
    
    async def scrape_with_library(self, origin: Optional[str] = None,
                                   destination: Optional[str] = None,
                                   departure_date: Optional[str] = None) -> Dict:
        """Scrape Azul using BeautifulSoup library"""
        try:
            ...
            for elem in price_elements:
                ...
                if 'pontos' in text.lower() or 'tudoazul' in text.lower():
                    price_miles = self.extract_price_miles_regex(text)
            
            # Search for TudoAzul points
            miles_elements = soup.find_all(text=lambda t: t and ('pontos' in t.lower() or 'tudoazul' in t.lower()))
    ...
    async def scrape_with_regex(self, origin: Optional[str] = None,
                                destination: Optional[str] = None,
                                departure_date: Optional[str] = None) -> Dict:
        """Scrape Azul using regex patterns"""
        try:
            html = await self.fetch_page(self.base_url)
